...
class ScaleMonitor:
    def __init__(self, root):
        ...
        self.ser = None
        self.connect_serial()
    def make_poll_cmd(self):
        mux = self.mux_id.get() if isinstance(self.mux_id, tk.Variable) else self.mux_id
        cmd = f"@08gl{mux}"
        checksum = calculate_checksum(cmd)  # XOR over everything in cmd (no CR)
        full_cmd = f"{cmd}{checksum}{COMMAND_TERMINATOR}"  # COMMAND_TERMINATOR is "\r"
        self.poll_cmd = full_cmd.encode("ascii")
        ...
        # Graph data arrays
        self.time_data = []
        self.weight_data = [[] for _ in range(4)]
        self.total_weight_data = []
    ...
    def create_ui(self):
        ...
        # new Count Overview tab
        count_tab = ttk.Frame(notebook)
        notebook.add(count_tab, text="Count Overview")
        self.create_count_ui(count_tab)

        # new Tracking Overview tab
    ...
    def create_control_ui(self, parent):
        ...
        self.num_cols = tk.IntVar(value=8)
        ...
        self.num_rows = tk.IntVar(value=4)
    ...
    def create_count_ui(self, parent):
        """Tab 2: Edit unit weight + big live object count."""
        frm = ttk.LabelFrame(parent, text="Object Unit Weight", padding=10)
        frm.pack(fill=tk.X, padx=10, pady=10)
        ttk.Label(frm, text="Weight (kg):").pack(side=tk.LEFT)
        entry = ttk.Entry(frm, textvariable=tk.StringVar(value=str(self.object_unit_weight)), width=10)
        entry.pack(side=tk.LEFT, padx=5)
        ttk.Button(frm, text="Set", command=lambda: self._set_weight_from_entry(entry)).pack(side=tk.LEFT)

        count_frame = ttk.Frame(parent, padding=20)
        count_frame.pack(expand=True, fill=tk.BOTH)
        lbl = ttk.Label(count_frame, textvariable=self.object_count_var,
                        font=("Arial", 90, "bold"))
        lbl.pack(expand=True)

    def _set_weight_from_entry(self, entry):
        """Shared helper to parse & set self.object_unit_weight from a given Entry."""
        try:
            w = float(entry.get())
            if w > 0:
    ...
    def create_tracking_ui(self, parent):
        """Tab 3: Edit unit weight + show COM & compartment plots."""
        frm = ttk.LabelFrame(parent, text="Object Unit Weight", padding=10)
        frm.pack(fill=tk.X, padx=10, pady=10)
        ttk.Label(frm, text="Weight (kg):").pack(side=tk.LEFT)
        entry = ttk.Entry(frm, textvariable=tk.StringVar(value=str(self.object_unit_weight)), width=10)
        ...
        plot_frame.pack(expand=True, fill=tk.BOTH, padx=10, pady=10)

        # position plot
        self.canvas_position = FigureCanvasTkAgg(self.fig_position, master=plot_frame)
        self.canvas_position.get_tk_widget().pack(side=tk.TOP, expand=True, fill=tk.BOTH)
    ...
    def reset_graphs(self):
        ...
        # --- Keep buffers as LISTS (not numpy arrays) ---
        self.time_data = []                       # was np.array([])
        self.weight_data = [[] for _ in range(4)] # was [np.array([]) ...]
        self.total_weight_data = []               # was np.array([])
        self.start_time = time.time()

        # Force next plot to refresh immediately
    ...
    def poll_weights(self):
        if self.running:
            ...
            self.root.after(int(POLLING_INTERVAL*1000), self.poll_weights)
            self.object_count_var.set(self.object_count)
    ...
    def request_weights(self):
        try:
            ...
            # 1) write the precomputed command
            # (it was created in self.make_poll_cmd() and updated when you press Connect)
            if self.poll_cmd is None:
                self.make_poll_cmd()
            # print(f"Sending: {self.poll_cmd!r}")  # <- keep commented for speed
            self.ser.write(self.poll_cmd)

            # 2) read one CR-terminated line quickly
            raw_response = self.ser.read_until(b"\r")
            # Some devices send CRLF; strip CR and optional LF
            if raw_response.endswith(b"\r\n"):
                raw_response = raw_response[:-2]
            elif raw_response.endswith(b"\r"):
            ...
            # print(f"Decoded Response: {response}")  # <- keep commented for speed
            ...
            for i in range(4):
                ...
                if len(self.weight_data[i]) < len(self.time_data):
                    self.weight_data[i].append(adjusted)
